...
class BaseExperiment(object):
    ...
    def display_method_info(self):
        ...
        elif self.args.method in ['lif','plif','tclif','lmh','stclif','stcplif','stclmh','glif','convlstm', 'predrnnpp', 'predrnn', 'mim', 'e3dlstm', 'mau']:
    ...
    def test(self):
        ...
        # print(results['trues'].shape)
        # n=0
        # # size1=results['preds'].shape
        # # size2=results['trues'].shape
        # results['preds']=np.expand_dims(results['preds'][:,n],axis=1)
        # results['trues']=np.expand_dims(results['trues'][:,n],axis=1)
        # metric_list=['mae', 'mse','ssim','psnr']
        ...
        if self._rank == 0:
            ...
            for np_data in ['metrics', 'inputs', 'trues', 'preds']:
                np.save(osp.join(folder_path, np_data + '.npy'), results[np_data][:50])
